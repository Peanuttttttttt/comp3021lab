...
public class Account {
	...
	public static Predicate<Account> checkBound = a -> {return(a.balance >=0 && a.balance<=10000); };
	...
	// TODO: Task3 
	// replace the null with a lambda expression
	public static AddMaker maker = n -> {return x->x.balance+=n;};
	...
	public static int getMaxAccountID(List<Account> accounts) {
		// TODO: Task4 
		// replace the null with a lambda expression
		//Account maxOne = accounts.stream().reduce(new Account(0, -100), (x1,x2) -> x1.balance<x2.balance?x2:x1);
		Account maxOne = accounts.stream().reduce(new Account(0, -100), (x1,x2) -> {if(x1.balance<x2.balance) {return x2;}else{
			return x1;
		}});
		return maxOne.id;
	}
